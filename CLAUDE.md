...
- **Titre** (`guessTitleFromLines`, `src/pdf/patternTextHeuristics.ts`) : la ligne de texte à la plus grande taille de police sur la première page — la métadonnée `Title` du PDF reflète souvent le nom du fichier, pas celui du patron, donc elle ne sert plus que de repli si l'heuristique ne trouve rien. Fonctionne pareil en français et en anglais : le signal est la taille de police, pas des mots-clés.
- **Créatrice/créateur** (`guessCreatorFromLines`) : reconnaît une ligne commençant par une tournure fixe ("Par…", "Conçu par…", "Un modèle de…", "Designed by…", "Pattern by…"…) — ancrée en début de ligne pour ne jamais matcher au milieu d'une phrase. Repli sur la métadonnée `Author` si rien ne correspond. Préremplit le champ "Source".
