...
- Recherche de texte : `PdfViewerCore` construit un index texte par page à la demande (première ouverture de la recherche, mis en cache pour la session de lecture) via `page.getTextContent()`, puis liste les pages contenant le terme cherché avec leur nombre d'occurrences — pas de surbrillance dans la page (pdf.js ne l'expose pas simplement hors de son propre viewer de référence), taper un résultat va directement à la page.
